...
const SpotsWeather = ({ spot }: { spot: SpotsInfoProps }) => {
  ...
  return (
    <section className="bg-white p-8 lg:p-8">
      ...
      {weather ? (
        <div className="mt-6 flex flex-col lg:flex-row-reverse gap-7">
          {/* Clima Atual - Estilizado */}
          <div className="flex flex-col w-full md:w-1/3  top-[100px] md:sticky h-fit bg-white bg-opacity-90 p-6 rounded-3xl shadow-xl backdrop-blur-lg border border-gray-200">
            {/* Cabeçalho */}
          ...
          <div className="w-full md:w-2/3 mt-6">
            ...
            {Object.entries(groupedForecasts).map(([date, forecasts]) => (
              <div key={date} className="mt-4">
                ...
                {/* 🔄 Linha de previsões */}
                <div className="flex gap-4 overflow-x-auto pb-4 snap-mandatory" style={{scrollSnapType: 'x mandatory'}}>
                  {/*@ts-ignore */}
        ...
        <p className="text-gray-500 mt-4">Carregando clima...</p>
      )}
    </section>
  );
};
